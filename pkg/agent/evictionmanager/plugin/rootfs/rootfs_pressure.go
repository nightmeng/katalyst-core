...
const (
	...
	metricsNamePodMinimumProtectionCount = "rootfs_pod_minimum_usage_protection_count"
...
func (r *PodRootfsPressureEvictionPlugin) ThresholdMet(_ context.Context) (*pluginapi.ThresholdMetResponse, error) {
	...
	isMinimumFreeThresholdMet := r.minimumFreeThresholdMet(config)
	isMinimumInodesFreeThresholdMet := r.minimumInodesFreeThresholdMet(config)
	r.Lock()
...
func (r *PodRootfsPressureEvictionPlugin) minimumFreeThresholdMet(config *eviction.RootfsPressureEvictionConfiguration) bool {
	...
	if config.MinimumFreeThreshold.Quantity != nil {
		...
		default:
			...
			if rate < float64(config.MinimumFreeThreshold.Percentage) {
				general.Infof("ThresholdMet result, Reason: MinimumFreeRateThreshold (Rate: %04f, Threshold: %04f)", rate, config.MinimumFreeThreshold.Percentage)
				return true
			}
		}
	}
...
func (r *PodRootfsPressureEvictionPlugin) minimumInodesFreeThresholdMet(config *eviction.RootfsPressureEvictionConfiguration) bool {
	...
	if config.MinimumInodesFreeThreshold.Quantity != nil {
		...
		default:
			...
			if rate < float64(config.MinimumInodesFreeThreshold.Percentage) {
				general.Infof("ThresholdMet result, Reason: MinimumInodesFreeRateThreshold (Rate: %04f, Threshold: %04f)", rate, config.MinimumInodesFreeThreshold.Percentage)
				return true
			}
		}
	}
...
func (r *PodRootfsPressureEvictionPlugin) GetTopEvictionPods(_ context.Context, request *pluginapi.GetTopEvictionPodsRequest) (*pluginapi.GetTopEvictionPodsResponse, error) {
	...
	if isMinimumFreeThresholdMet {
		pods, err = r.getTopNPods(request.ActivePods, request.TopN, config.PodMinimumUsedThreshold, config.ReclaimedQoSPodUsedPriorityThreshold, r.getPodRootfsUsed)
	} else if isMinimumInodesFreeThresholdMet {
		pods, err = r.getTopNPods(request.ActivePods, request.TopN, config.PodMinimumInodesUsedThreshold, config.ReclaimedQoSPodInodesUsedPriorityThreshold, r.getPodRootfsInodesUsed)
	}
	if err != nil {
...
func (r *PodRootfsPressureEvictionPlugin) getTopNPods(pods []*v1.Pod, n uint64, minUsedThreshold, reclaimedPodPriorityUsedThreshold *evictionapi.ThresholdValue, getPodRootfsUsageFunc getPodRootfsUsageFunc) ([]*v1.Pod, error) {
	...
	for i := range pods {
		...
		if err != nil {
			...
			usageItem.priority = r.checkReclaimedPodPriority(pods[i], used, percentage, reclaimedPodPriorityUsedThreshold)
			...
			if usageItem.priority {
				...
				if r.checkPodMinimumUsage(used, percentage, minUsedThreshold) {
					general.Infof("MinimumUsageProtection: %s (used: %d, percentage: %04f)", pods[i].UID, used, percentage)
					_ = r.emitter.StoreInt64(metricsNamePodMinimumProtectionCount, 1, metrics.MetricTypeNameCount,
						metrics.ConvertMapToTags(map[string]string{
							metricsTagKeyUsed:       fmt.Sprintf("%d", used),
							metricsTagKeyPercentage: fmt.Sprintf("%04f", percentage),
						})...)
