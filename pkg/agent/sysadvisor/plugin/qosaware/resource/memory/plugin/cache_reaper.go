...
func (cp *cacheReaper) selectContainers(containers []*types.ContainerInfo, cacheToReap resource.Quantity, numaID int, metricName string) []*types.ContainerInfo {
	general.NewMultiSorter(func(s1, s2 interface{}) int {
		...
		expireAt := utils.GetMetricExpireTimestamp(cp.conf.DynamicAgentConfiguration)
		c1Metric, c1Err := helper.GetContainerMetric(cp.metaServer.MetricsFetcher, cp.emitter, c1.PodUID, c1.ContainerName, metricName, numaID, expireAt)
		c2Metric, c2Err := helper.GetContainerMetric(cp.metaServer.MetricsFetcher, cp.emitter, c2.PodUID, c2.ContainerName, metricName, numaID, expireAt)
	...
	for _, ci := range containers {
		metric, err := helper.GetContainerMetric(cp.metaServer.MetricsFetcher, cp.emitter, ci.PodUID, ci.ContainerName, metricName, numaID, utils.GetMetricExpireTimestamp(cp.conf.DynamicAgentConfiguration))
		if err != nil {
