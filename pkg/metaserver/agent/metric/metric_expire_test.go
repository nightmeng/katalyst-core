...
func TestMetricExpire(t *testing.T) {
	updateTime := time.Now().Add(-35 * time.Second)
	...
	checkMetricDataExpire = checkMetricDataExpireFunc(34 * time.Second)
	_, err = checkMetricDataExpire(metricData, nil)
	assert.Equal(t, errMetricDataExpired, err)

	checkMetricDataExpire = checkMetricDataExpireFunc(36 * time.Second)
	_, err = checkMetricDataExpire(metricData, nil)
	assert.NoError(t, err)
